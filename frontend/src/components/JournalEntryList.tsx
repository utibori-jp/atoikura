...
function InlineEditForm({ entry, category_groups, all_expense_categories, onSave, onCancel }: EditFormProps) {
  ...
  const handle_submit = async (e: React.FormEvent) => {
    ...
    if (!selected_category_id) { setErrorMessage('生活区分を選択してください'); return; }
    if (isNaN(parsed_amount) || parsed_amount < 1) { setErrorMessage('金額は1以上の整数を入力してください'); return; }
    setSubmitting(true);
    try {
      await api.updateJournalEntry(entry.id, {
        transaction_date, amount: parsed_amount,
        category_id: parseInt(selected_category_id, 10),
        is_excluded, item: item || null, note: note || null,
      });
      onSave();
    } catch (err) {
  ...
  return (
    <form onSubmit={handle_submit} style={{
      padding: 18, background: T.bgSoft, borderRadius: 16,
      border: `1.5px solid ${T.hair}`, margin: '6px 0',
    }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <select value={selected_category_id} onChange={(e) => setSelectedCategoryId(e.target.value)} disabled={!selected_group_id} style={{ opacity: selected_group_id ? 1 : 0.5 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, color: T.inkSoft, fontWeight: 600, display: 'block', marginBottom: 4 }}>メモ</label>
        <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="メモを入力..." rows={2} style={{ resize: 'vertical' }} />
      </div>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: T.inkSoft, cursor: 'pointer', marginBottom: 14 }}>
        <input type="checkbox" checked={is_excluded} onChange={(e) => setIsExcluded(e.target.checked)} style={{ width: 'auto' }} />
        集計から除外
      </label>
      {error_message && <p style={{ color: T.coralDeep, fontSize: 12, marginBottom: 10 }}>{error_message}</p>}
      <div style={{ display: 'flex', gap: 8 }}>
        <button type="submit" disabled={submitting} style={{
          border: 'none', background: T.coral, color: '#fff',
          padding: '8px 20px', borderRadius: 999, fontFamily: 'inherit',
          fontWeight: 700, fontSize: 13, cursor: submitting ? 'default' : 'pointer',
          opacity: submitting ? 0.7 : 1, boxShadow: `0 3px 0 ${T.coralDeep}`,
        }}>
          {submitting ? '保存中...' : '保存'}
        </button>
        <button type="button" onClick={onCancel} style={{
          background: 'transparent', border: `1.5px solid ${T.hair}`,
          borderRadius: 999, color: T.inkSoft, padding: '8px 18px',
...
export function JournalEntryList({ year_month, refresh_token }: Props) {
  ...
  const monthly_total = entries
    ? entries.flatMap((d) => d.journal_entries).filter((e) => !e.is_excluded).reduce((sum, e) => sum + e.amount, 0)
    : 0;
  ...
  return (
    <div>
      {/* Monthly total pill */}
      <div style={{
        background: "#DEF1E6", borderRadius: 24, padding: "18px 24px",
        marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      }}>
        <span style={{ fontSize: 14, color: T.sageDeep, fontWeight: 700 }}>今月の合計（変動費）</span>
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 26, fontWeight: 700, color: T.ink, letterSpacing: '-0.5px' }}>
      ...
      {entries !== null && entries.map((daily) => {
        ...
        return (
          <div key={daily.date} style={{
            background: T.card, borderRadius: 28, marginBottom: 16,
            boxShadow: T.cardShadow, overflow: 'hidden',
          }}>
            {/* Day header */}
            <div style={{ padding: '16px 20px 0', borderBottom: `1.5px solid ${T.hair}`, paddingBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{
                  ...
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 22, lineHeight: 1, color: T.ink }}>{day}</span>
                  <span style={{ fontSize: 10, color: T.inkSoft, marginTop: 2 }}>{weekday}曜日</span>
                </div>

                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 18, marginLeft: 'auto', color: T.ink }}>
                  ...
                  {note && editing_note_date !== daily.date && (
                    <div style={{
                      ...
                      background: '#FFF6E5', border: `1px dashed ${T.mustard}`,
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 700, fontSize: 12, cursor: saving_note_date === daily.date ? 'default' : 'pointer',
                            boxShadow: `0 3px 0 ${T.coralDeep}`, opacity: saving_note_date === daily.date ? 0.7 : 1,
                          }}
                        >
                        ...
                        <button
                          ...
                          style={{
                            background: 'transparent', border: `1.5px solid ${T.hair}`,
                            borderRadius: 999, color: T.inkSoft, padding: '6px 14px',
              ...
              <div key={entry.id}>
                {editing_entry?.id === entry.id ? (
                  ...
                  <div style={{
                    ...
                    borderBottom: idx < daily.journal_entries.length - 1 ? `1px solid ${T.hair}` : 'none',
                    gap: 12,
                  }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <span style={{ fontWeight: 600, fontSize: 14, color: entry.is_excluded ? T.inkSoft : T.ink }}>
                          {entry.item ?? '—'}
                        </span>
                        <span style={{
                          background: T.bgSoft, borderRadius: 6, fontSize: 11,
                          padding: '2px 8px', color: T.inkSoft, border: `1px solid ${T.hair}`,
                        ...
                        {entry.is_excluded && (
                          <span style={{
                            background: T.excluded, borderRadius: 6, fontSize: 11,
                            padding: '2px 8px', color: '#fff', fontWeight: 700,
                          }}>対象外</span>
                        )}
                      </div>
                      {entry.note && (
                        <div style={{ fontSize: 12, color: T.inkSoft, marginTop: 3 }}>{entry.note}</div>
                      )}
                    </div>
