    ...
    >
      ...
      >
        ...
        <div style={{ background: T.bgSoft, border: `1.5px solid ${T.hair}`, borderRadius: 14, padding: "12px 14px", marginBottom: 10 }}>
          ...
          <div style={{ fontWeight: 600, fontSize: 14, color: T.inkSoft, fontStyle: "italic" }}>例：新しいカメラ</div>
        </div>

        {/* アイコン */}
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "12px 14px", background: T.bgSoft, border: `1.5px solid ${T.hair}`, borderRadius: 14 }}>
          <div style={{ fontSize: 11, color: T.inkSoft }}>アイコン</div>
          <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
            {EMOJI_OPTIONS.map((e, i) => (
              <span
                key={e}
                style={{ width: 34, height: 34, borderRadius: 10, background: i === 0 ? T.mustardSoft : "#fff", border: `1.5px solid ${i === 0 ? T.mustard : T.hair}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17 }}
              >
                {e}
              </span>
            ))}
        ...
        {/* 毎月の積立額 */}
        <div style={{ background: T.bgSoft, border: `1.5px solid ${T.mustard}`, borderRadius: 18, padding: "14px 18px", display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
          <div style={{ flex: 1, background: T.bgSoft, border: `1.5px solid ${T.hair}`, borderRadius: 14, padding: "12px 14px" }}>
            ...
            <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 17, color: T.inkSoft }}>—</div>
          </div>
          <div style={{ flex: "0 0 138px", background: T.bgSoft, border: `1.5px solid ${T.hair}`, borderRadius: 14, padding: "12px 14px" }}>
            <div style={{ fontSize: 11, color: T.inkSoft, marginBottom: 3 }}>目標日（任意）</div>
            <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 14, color: T.inkSoft }}>YYYY/MM</div>
          </div>
        </div>

        {/* メモ */}
        <div style={{ background: T.bgSoft, border: `1.5px solid ${T.hair}`, borderRadius: 14, padding: "12px 14px", marginBottom: 18, minHeight: 64 }}>
          <div style={{ fontSize: 11, color: T.inkSoft, marginBottom: 3 }}>メモ（任意）</div>
          <div style={{ fontSize: 13, color: T.inkSoft, fontStyle: "italic" }}>例：旅行記録用に。レンズ込みで揃えたい。</div>
        </div>
  ...
  return (
    <div>
      ...
      {!is_loading && goals && (
        <>
          {/* Hero card */}
          <div style={{ background: T.card, borderRadius: 28, padding: "22px 20px", boxShadow: "0 0 0 1px rgba(80,40,10,0.05), 0 6px 32px rgba(80,40,10,0.09)", marginBottom: 20 }}>
            <div style={{ fontSize: 10, color: T.inkSoft, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>今月の貯金合計</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginTop: 8 }}>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 800, fontSize: 40, letterSpacing: "-0.04em", color: T.coral, lineHeight: 1 }}>
                ¥{yenSlim(savings_total)}
              </span>
            </div>
            <div style={{ fontSize: 11, color: T.inkSoft, marginTop: 5, fontWeight: 500 }}>毎月自動で記録されます</div>
            <div style={{ display: "flex", gap: 6, marginTop: 16 }}>
              {goals.map((g) => (
                <span
                  key={g.id}
                  style={{ padding: "5px 10px", borderRadius: 999, background: T.bgSoft, border: `1px solid ${T.hair}`, color: T.ink, fontSize: 10, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 4 }}
                >
                  <span style={{ fontSize: 11 }}>{g.emoji}</span>
                  <span style={{ fontFamily: "'DM Sans', sans-serif", color: T.inkSoft }}>{yenSlim(g.monthly_amount / 1000)}k</span>
                </span>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", margin: "0 2px 10px" }}>
            ...
            <span style={{ marginLeft: "auto", fontSize: 11, color: T.inkSoft }}>{goals.length}件</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {goals.map((g) => {
              ...
              return (
                <div key={g.id} style={{ background: T.card, borderRadius: 28, padding: "16px", boxShadow: T.cardShadow }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 13, background: T.mustardSoft, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
                      {g.emoji}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{g.name}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                        <span style={{ padding: "3px 9px", borderRadius: 999, background: T.mustardSoft, color: T.mustardDeep, fontSize: 10, fontWeight: 700, fontFamily: "'DM Sans', sans-serif" }}>
                          ¥{yenSlim(g.monthly_amount)}/月
                        </span>
                        {g.is_posted_this_month ? (
                          <span style={{ padding: "3px 9px", borderRadius: 999, background: T.sageSoft, color: T.sageDeep, fontSize: 10, fontWeight: 700 }}>✅ 今月済</span>
                        ) : (
                          <span style={{ padding: "3px 9px", borderRadius: 999, background: T.mustardSoft, color: T.mustardDeep, fontSize: 10, fontWeight: 700 }}>⏳ 今月分待ち</span>
                        )}
                      </div>
                    </div>
                  ...
                  {g.memo && (
                    <div style={{ marginTop: 12, padding: "9px 12px", borderRadius: 10, background: T.bgSoft, fontSize: 12, color: T.inkSoft, lineHeight: 1.5 }}>
                      💭 {g.memo}
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${T.hair}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 11, color: T.inkSoft }}>
                        目標 <strong style={{ fontFamily: "'DM Sans', sans-serif", color: T.ink }}>¥{yenSlim(g.target_amount)}</strong>
                        {g.deadline ? ` / ${g.deadline}` : ""}
                      </span>
                    </div>
                    <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 7 }}>
                      <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 20, color: T.mustardDeep }}>
                        ¥{yenSlim(g.accumulated_amount)}
                      </span>
                      <span style={{ fontSize: 11, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>/ {pct}%</span>
                    </div>
                    <div style={{ height: 7, background: T.bgSoft, borderRadius: 999, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${T.mustard}, ${T.coral})`, borderRadius: 999 }} />
                    </div>
                  </div>
                </div>
              );
