    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: T.inkSoft, marginTop: 4 }}>目的別に積み立てて、着実に前進</div>
        </div>
        <button style={{
      ...
      {/* Summary hero */}
      <div style={{
        background: T.card, borderRadius: 32, padding: "22px 28px",
        boxShadow: "0 8px 24px -16px rgba(80,40,10,0.18)",
        display: "flex", alignItems: "center", gap: 32,
      }}>
        <div>
          <div style={{ fontSize: 12, color: T.inkSoft, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
            今月の貯金合計
          </div>
        ...
        {goals.map((g) => {
          ...
          return (
            <div key={g.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5, flexWrap: "wrap" }}>
                    <span style={{
                      ...
                      background: "#FFF1CC", color: "#A3791F",
                      fontSize: 11, fontWeight: 700, fontFamily: "'DM Sans', sans-serif",
                    }}>
                      ¥{yenSlim(g.monthly_amount)}/月
                    </span>
                    {g.is_posted_this_month ? (
                      <span style={{ padding: "3px 10px", borderRadius: 999, background: "#DEF1E6", color: T.sageDeep, fontSize: 11, fontWeight: 700 }}>
                        ✅ 今月済
                      </span>
                    ) : (
                      <span style={{ padding: "3px 10px", borderRadius: 999, background: "#FFF1CC", color: "#A3791F", fontSize: 11, fontWeight: 700 }}>
                        ⏳ 今月待ち
                      </span>
                    )}
                  </div>
              ...
              {g.memo && (
                <div style={{ padding: "10px 14px", borderRadius: 12, background: T.bgSoft, fontSize: 13, color: T.inkSoft, lineHeight: 1.5 }}>
                  💭 {g.memo}
                </div>
              )}

              <div style={{ padding: "14px 16px", borderRadius: 16, background: T.bgSoft, border: `1.5px solid ${T.hair}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
                  <span style={{ fontSize: 11, color: T.inkSoft, fontWeight: 600 }}>積立累計</span>
                  <span style={{ fontSize: 11, color: T.inkSoft }}>
                    目標 <strong style={{ fontFamily: "'DM Sans', sans-serif", color: T.ink }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 8 }}>
                  <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 22, color: "#A3791F" }}>
                    ¥{yenSlim(g.accumulated_amount)}
                  </span>
                  <span style={{ fontSize: 12, color: T.inkSoft, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>
                    / {pct}%
                  </span>
                </div>
              ...
              {!g.is_posted_this_month && (
                <button
                  onClick={() => api.postMonthlySavings(g.id, g.monthly_amount, ym)
                    .then(() => api.listSavingsGoals().then((r) => setGoals(r.savings_goals)))
                    .catch(() => {})}
                  style={{
                    border: "none", background: T.mustard, color: T.ink,
