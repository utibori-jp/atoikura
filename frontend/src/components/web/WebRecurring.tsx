...
export function WebRecurring({ onBack }: Props) {
  ...
  const handle_confirm = async (id: number) => {
    ...
    try {
      ...
      setRecurring(rec_res.recurring_expenses);
      setPending(pend_res.pending);
      setConfirmAmounts((prev) => { const n = { ...prev }; delete n[id]; return n; });
    } catch {
      // ignore
    } finally {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: T.inkSoft, marginTop: 4 }}>毎月の固定・半固定費を管理します</div>
        </div>
        <button style={{
      ...
      {pending.length > 0 && (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <span style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontWeight: 900, fontSize: 15, color: T.coralDeep }}>
              💬 確認待ち
            </span>
            <span style={{
          ...
          <div style={{ display: "flex", gap: 16 }}>
            {pending.map((p) => (
              <div key={p.id} style={{
                flex: 1, background: T.card, borderRadius: 24, padding: "20px 22px",
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{
                        ...
                        background: T.coral, color: "#fff", fontSize: 11, fontWeight: 700,
                      }}>要確認</span>
                    </div>
                    <div style={{ fontSize: 12, color: T.inkSoft, marginTop: 4 }}>
                      前回 <strong style={{ fontFamily: "'DM Sans', sans-serif", color: T.ink }}>
                        {yen(p.last_amount)}
                      </strong> · 毎月 {p.billing_day}日
                    </div>
                  </div>
                </div>
                <div style={{ marginTop: 12, display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    ...
                    onChange={(e) => setConfirmAmounts((prev) => ({ ...prev, [p.id]: e.target.value }))}
                    style={{
                      flex: 1, padding: "10px 12px",
                      border: `1.5px solid ${T.hair}`, borderRadius: 10,
                      fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 700,
                      outline: "none", background: T.bgSoft,
                    }}
                  />
                  <button
                    ...
                    style={{
                      flex: 1, border: "none", background: T.coral, color: "#fff",
                      padding: "12px", borderRadius: 14,
                      fontFamily: "inherit", fontWeight: 700, fontSize: 14,
                      boxShadow: `0 4px 0 ${T.coralDeep}`, cursor: "pointer",
                    }}
                  >
                    金額を確定する
                  </button>
      ...
      <div style={{ background: T.card, borderRadius: 32, padding: 28, boxShadow: "0 8px 24px -16px rgba(80,40,10,0.18)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
          <div style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontSize: 16, fontWeight: 700 }}>
            繰り返し設定&nbsp;
            <span style={{ fontSize: 12, color: T.inkSoft, fontWeight: 400 }}>{filtered.length}件</span>
          </div>
            ...
            return (
              <div key={r.id} style={{
                ...
                <div style={{
                  ...
                  fontSize: 22, boxShadow: `0 2px 0 ${T.hair}`,
                }}>{r.emoji}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                    <span style={{
                      ...
                      background: "#E5EEF7", color: "#3F6B91",
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 16, color: r.amount ? T.ink : T.inkSoft }}>
        ...
        <button style={{
          ...
          border: `1.5px dashed ${T.coral}`, background: "transparent",
          color: T.coralDeep, padding: "14px", borderRadius: 16,
