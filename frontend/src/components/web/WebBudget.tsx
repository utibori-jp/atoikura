...
export function WebBudget({ onNavigate }: Props) {
  ...
  const spent_pct = summary.variable_budget > 0
    ? Math.round(((summary.variable_budget - Math.max(0, summary.variable_budget - 0)) / summary.variable_budget) * 100)
    : 0;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        ...
        <div style={{ padding: "8px 14px", borderRadius: 999, background: T.card, fontSize: 12, color: T.inkSoft }}>
      ...
      {/* Hero card */}
      <div style={{
        background: T.card, borderRadius: 32, padding: 32,
        ...
        <div style={{ flex: "0 0 480px", paddingRight: 32, borderRight: `1.5px solid ${T.hair}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 12, color: T.inkSoft, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              今月の予算
            </span>
            <span style={{
              padding: "2px 8px", borderRadius: 999,
              background: T.bgSoft, border: `1px solid ${T.hair}`,
              fontSize: 10, color: T.inkSoft, fontWeight: 700,
            }}>自動</span>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 12 }}>
            <span style={{
              ...
              letterSpacing: "-0.04em", color: T.coral, lineHeight: 1,
            }}>
              {yenSlim(summary.variable_budget)}
            </span>
            <span style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontWeight: 700, fontSize: 24, color: T.ink }}>円</span>
          </div>
          <div style={{ fontSize: 13, color: T.inkSoft, marginTop: 8 }}>変動費に使える金額（今月分）</div>
          <div style={{ height: 8, background: T.bgSoft, borderRadius: 999, marginTop: 16, overflow: "hidden" }}>
            <div style={{
              height: "100%", width: `${Math.min(100, spent_pct)}%`,
              background: `linear-gradient(90deg,${T.mustard},${T.coral})`,
              borderRadius: 999,
            }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.inkSoft, marginTop: 6, fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ flex: 1, paddingLeft: 0, display: "grid", gridTemplateColumns: "1fr 1fr", alignContent: "center" }}>
          {[
            { label: "1日あたり", value: `¥${yenSlim(summary.daily_budget)}`, sub: `÷ ${days_total}日` },
            { label: "今月の残り", value: `${summary.days_remaining}日`, sub: `${parseInt(ym.split("-")[1])}/${today_day} 時点` },
            { label: "消化ペース", value: `${spent_pct}%`, sub: "進行中" },
            { label: "収入 − 固定", value: `¥${yenSlim(summary.income_total - summary.recurring_total)}`, sub: "変動費 + 貯金の元" },
          ].map((s, i) => (
            <div key={s.label} style={{
              ...
              <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 800, fontSize: 26, marginTop: 4, color: T.ink, letterSpacing: "-0.02em" }}>
                {s.value}
              </div>
      ...
      <div style={{ display: "flex", gap: 16 }}>
        {tiles.map((t) => (
          <div
            ...
            style={{
              flex: 1, background: T.card, borderRadius: 28, padding: "22px 24px",
              boxShadow: "0 8px 24px -16px rgba(80,40,10,0.18)",
            ...
            onMouseEnter={(e) => {
              ...
              (e.currentTarget as HTMLDivElement).style.boxShadow = "0 12px 32px -12px rgba(80,40,10,0.22)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLDivElement).style.transform = "translateY(0)";
              (e.currentTarget as HTMLDivElement).style.boxShadow = "0 8px 24px -16px rgba(80,40,10,0.18)";
            }}
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontWeight: 700, fontSize: 16 }}>{t.title}</div>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
              <span style={{
                ...
                color: t.tone.fg, padding: "2px 8px", borderRadius: 6, background: t.tone.signBg,
              }}>{t.sign}</span>
              <span style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 800, fontSize: 32, letterSpacing: "-0.03em", color: T.ink }}>
                {yen(t.amount)}
              </span>
            </div>
            <div style={{ fontSize: 11, color: T.inkSoft, marginTop: 3 }}>今月の合計</div>
            <div style={{ position: "absolute", bottom: 18, right: 20, fontSize: 22, color: T.coral, opacity: 0.55 }}>›</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        background: T.bgSoft, border: `1px dashed ${T.hair}`,
        ...
        <span style={{ color: "#3F6B91", fontWeight: 700 }}>定期支出 {yen(summary.recurring_total)}</span>
        <span style={{ color: T.inkSoft }}>−</span>
      ...
      <div style={{ background: T.card, borderRadius: 32, padding: 28, boxShadow: "0 8px 24px -16px rgba(80,40,10,0.18)" }}>
        <div style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontSize: 16, fontWeight: 700, marginBottom: 16 }}>
          直近3ヶ月の予算
        </div>
        <div style={{ display: "flex", gap: 16 }}>
          {summary.history.map((r) => {
            ...
            return (
              <div key={r.year_month} style={{
                flex: 1, padding: "18px 20px", borderRadius: 24,
                background: ongoing ? "#FFF1CC" : T.bgSoft,
                border: `1.5px solid ${ongoing ? T.mustard : T.hair}`,
              }}>
                <div style={{ fontFamily: "'Zen Maru Gothic', 'M PLUS Rounded 1c', sans-serif", fontWeight: 700, fontSize: 17 }}>
                  {monthLabel(r.year_month)}
                  {ongoing && (
                    <span style={{ fontSize: 11, marginLeft: 6, color: T.coralDeep, fontWeight: 700 }}>進行中</span>
                  )}
                </div>
                <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 22, marginTop: 8 }}>
                  {yen(r.actual)}
                </div>
